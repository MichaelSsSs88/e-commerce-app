...
export class BillEditComponent implements OnInit, OnDestroy {
  ...
  constructor(private billService: BillService, private router: Router, private activeRoute: ActivatedRoute, private customerService: CustomerService, private calculateService: CalculateService) {
    ...
    !this.billSeleted && this.router.url.match("edit") && this.router.navigate(['bill']);
  ...
  inicializarForm(): FormGroup {
    ...
    return new FormGroup({
      ...
      datePayment: new FormControl(new Date().toLocaleDateString(), [Validators.required, this.validateIdentification]),
      ...
      amount: new FormControl(0, [Validators.required/*,this.validateDescription*/]),
      paymentDescription: new FormControl("", [Validators.required, this.validateName]),
    });
  }
  ngOnInit(): void {
  ...
  applyPayment() {
    this.isPaymentSectionActive?this.billForm.controls["totalPrice"].setValue(this.billSeleted?.total):this.billForm.controls["totalPrice"].setValue(this.getTotal());
    this.isPaymentSectionActive = !this.isPaymentSectionActive;
  }

  addPayment() {
    if(this.billSeleted&&((this.getTotal()+ +this.billForm.value.amount)<= this.billSeleted?.total)){
      this.billSeleted?.payments.push(new PaymentModel(this.billForm.value.paymentDescription, this.billForm.value.amount, this.billForm.value.datePayment));
      this.billForm.controls["totalPrice"].setValue(this.getTotal());
    }else{
        alert("The payment can not be applied")
    }
  }

  getTotal(): number {
    let total = 0;
    this.billSeleted?.payments.forEach(payment => {total += payment.amount});
    return total;
  }
  ...
  onSubmit(): void {
    ...
    if (this.buttonMessage == "Update") {
      ...
      this.billSeleted?.customer && this.billService.updateBill(new BillModel(this.billForm.value.billNumber, this.billSeleted?.customer, this.billLinesList, this.billForm.value.dolarToColonValue, this.billForm.value.totalPrice, this.billSeleted.payments, this.billForm.value.date));
    }
  }
